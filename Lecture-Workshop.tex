...
\item Easy to code in WinBUGS ({\color{red}JAGS?})
\item Allows each parameter to have own precision with mean 1
\item Usually re-scale $X$ so that columns have mean 0 and standard deviation $1$. 
\end{itemize}
...
\begin{equation*}
\begin{aligned}
{\bf Y}|\beta,\sigma^2 &\sim N({\bf X\beta},\sigma^2{\bf I_n})\\
...
For $\theta_j$
\begin{itemize}
\item propose a new $\theta_j^*\sim q(\cdot | \theta_j^t)$
\item Sampling from the wrong distribution!
\item Need to balance acceptance of proposed values so that we have samples that represent the posterior
\item Metropolis-Hastings ratio
\[
...
\begin{frame}{Air Pollution Data ({\color{red} KENT: REPLACE WITH YOUR EXAMPLE?)}}
\begin{itemize}
